...
def evaluate(model, graph, device, data_iter, split='valid'):
    ...
    for k in [1, 3, 10]:
        ...
        results['hits@{}'.format(k)] = round(
            (left_results['hits@{}'.format(k)] + right_results['hits@{}'.format(k)]) / (2 * count), 5)
    return results
...
def main(args):
    ...
    print("Test MRR: {:.5}\n, MR: {:.10}\n, H@10: {:.5}\n, H@3: {:.5}\n, H@1: {:.5}\n" \
          .format(test_results['mrr'], test_results['mr'], test_results['hits@10'], test_results['hits@3'],
                  test_results['hits@1']))
...
from CompGCN_Att.data_loader import Data
import numpy as np
